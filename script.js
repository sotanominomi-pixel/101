...
/* --- 【追加改善】デバイス切り替え & 吹き出し全体ドラッグ対応ロジック --- */
...
renderClockMode = function() {
    ...
    if (pipContainer && deviceMode === 'mobile') {
        pipContainer.innerHTML = `
            <button onclick="toggleFloatingClock()" class="action-button pip-btn" style="background-color: #f39c12;">
                ${currentLang === 'ja' ? 'フローティング時計を表示' : 'Show Floating Clock'}
            </button>
        `;
    }
};
...
renderSettingsMode = function() {
    ...
    if (list) {
        const li = document.createElement('li');
        li.innerHTML = `
            <span>${currentLang === 'ja' ? 'デバイス設定' : 'Device Mode'}</span>
            <div class="segmented-control">
                <button id="mode-pc" class="segment-button ${deviceMode === 'pc' ? 'active' : ''}">PC</button>
                <button id="mode-mobile" class="segment-button ${deviceMode === 'mobile' ? 'active' : ''}">Mobile</button>
            </div>
        `;
...
updateClock = function() {
    ...
    const clockDisplay = document.getElementById('n-clock-display');
    if (fDisplay && clockDisplay) {
        fDisplay.textContent = clockDisplay.textContent;
    }
};
...
/* --- 【追加改善アドオン】機能を拡張する追加ロジック（最新修正版） --- */

// 1. ダークモード自動適応
const applyDarkMode = (isDark) => {
    document.body.style.backgroundColor = isDark ? '#121212' : '#fff';
    document.body.style.color = isDark ? '#fff' : '#000';
    document.querySelectorAll('.tab-bar, .alarm-item, .settings-list li, .preset-item, .accordion-header').forEach(el => {
        el.style.backgroundColor = isDark ? '#1e1e1e' : '#f7f7f7';
        el.style.borderColor = isDark ? '#333' : '#ccc';
    });
...
darkModeMediaQuery.addEventListener('change', e => applyDarkMode(e.matches));
applyDarkMode(darkModeMediaQuery.matches);

// 2. 触覚フィードバック
document.addEventListener('click', (e) => {
    if (e.target.closest('button') || e.target.closest('.tab-item') || e.target.closest('.slider')) {
        if (navigator.vibrate) navigator.vibrate(10);
    }
});

// 3. ダイナミック・ファビコン
const updateDynamicFavicon = () => {
    const canvas = document.createElement('canvas');
    canvas.width = 32; canvas.height = 32;
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = currentN === 24 ? '#000' : '#f39c12';
    ctx.beginPath(); ctx.arc(16, 16, 16, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = 'bold 14px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    const now = new Date();
    const realTimeOfDay = now.getTime() - new Date(now.toDateString()).getTime();
    const { h } = calculateNTime(realTimeOfDay);
...
setInterval(updateDynamicFavicon, 60000);
updateDynamicFavicon();

// 4. 【不具合修正】画面遷移しても時計の計算・表示を維持する最優先ロジック
const finalUpdateClock = updateClock;
updateClock = function() {
    finalUpdateClock();
    const now = new Date();
    const realTimeOfDay = now.getTime() - new Date(now.toDateString()).getTime();
    ...
    const formattedS = String(s).padStart(2, '0');
    let timeString = `${formattedH}:${formattedM}`;
    if (isSecondsVisible) timeString += `:${formattedS}`;

    const fDisplay = document.getElementById('floating-display');
    if (fDisplay) fDisplay.textContent = timeString;

    if (!document.getElementById('n-clock-display') && typeof pipCtx !== 'undefined') {
        ...
        pipCtx.fillText(timeString, pipCanvas.width/2, pipCanvas.height/2);
        pipCtx.font = "20px Roboto"; pipCtx.fillText(`N=${currentN}`, pipCanvas.width/2, pipCanvas.height/2 + 50);
    }
};

/* --- 【スマホ別アプリ背面動作のための動画ストリーム PiP アドオン】 --- */

// 既存の togglePiP を上書きしてストリーム制御を注入
const originalTogglePiP = togglePiP;
togglePiP = async function() {
    ...
    try {
        ...
        console.error('PiP Stream Error:', error);
        alert('PiPの起動に失敗しました。');
    }
};

// 背面でも描画が止まらないための専用描画ループ
const streamCanvasUpdate = () => {
    if (!pipCtx) return;
    const now = new Date();
    const realTimeOfDay = now.getTime() - new Date(now.toDateString()).getTime();
    const { h, m, s } = calculateNTime(realTimeOfDay);
    const timeText = `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}${isSecondsVisible ? ':' + String(s).padStart(2, '0') : ''}`;
    
    pipCtx.fillStyle = "black";
    pipCtx.fillRect(0, 0, pipCanvas.width, pipCanvas.height);
    pipCtx.fillStyle = "white";
    pipCtx.font = "bold 70px 'Courier New', monospace";
    pipCtx.textAlign = "center";
    pipCtx.textBaseline = "middle";
    pipCtx.fillText(timeText, pipCanvas.width / 2, pipCanvas.height / 2 - 10);
    pipCtx.font = "24px Arial";
    pipCtx.fillText(`N = ${currentN}`, pipCanvas.width / 2, pipCanvas.height / 2 + 45);
};
setInterval(streamCanvasUpdate, 100);
